...
<style type="text/css">
  ...
  <script type="text/javascript">
  	$(function(){
  		$("#dg").datagrid({
 			...
 			toolbar: [{
 				...
 				text:'添加信息',
 				handler: function(){
 					createwindow("添加信息", "bookSaleController.do?goadd&id=save",400,300);
 					}
 			},{
 				iconCls: 'icon-edit',
 				text:'编辑信息',
  			 ...
  			 columns:[[
					{field : 'IDs',title : 'IDs',checkbox : true,width : 8,align : 'center'}, 
  			         ...
  			         {field:'BNAME',title:'教材名',width:60,align:'center'},   
  			         {field:'SNUM',title:'出售数量',width:60,align:'center'},   
  			         {field:'TIME',title:'出售时间',width:60,align:'center'},   
  	...
  	function deletes(id){
  		var url ="bookSaleController.do?deletes&id="+id;
			$.ajax({
				url:url,
				dataType:'json',
  	...
  	function update(){
  		...
  		if(rows.length!=1){
  			...
  			var url = "bookSaleController.do?goadd&id="+rows[0].ID;
  	...
  	function saledata(){
  		$.ajax({
  			url:'bookSaleController.do?BookSaleData',
  			dataType:'json',
